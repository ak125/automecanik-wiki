...
1. **Contradiction inter-sources : bloquée implicitement, pas signalée explicitement.** La corroboration
   par bucket `(grandeur, unité, valeur)` fait qu'une divergence (Brembo `0,10` / Textar `0,07` / ZF `0,05`
   pour le faux-rond) tombe dans des buckets DISJOINTS → chaque valeur reste isolée (corroboration 1) →
   `numeric_ambiguous`. La divergence ne peut donc **jamais** produire un faux PASS (fail-safe). Ce qui
   reste **V2** = l'émission d'un `disputed_claim` EXPLICITE (signaler « ces 3 sources se contredisent »
   plutôt que « valeur non corroborée ») ; `no_disputed_claims` reste donc `NOT_YET_WIRED`.
...
1. **Seed `numeric-plausibility.yaml` = 100 % `proposed`** : le pilote confère donc **0 `numeric_verified`**
   ...
   La corroboration réelle est désormais **comptée** depuis les sources CAPTÉES distinctes concordantes
   (`classify_all`, plus de stub à 1 ; le pipeline joint `source_id` = slug catalogué). `numeric_verified`
   exige donc (a) une plage `validated`, (b) une page `captured`/`verified`, (c) **≥2 sources captées
   distinctes concordantes**. Aujourd'hui 0 page captée → corroboration 0 → toute plage validée reste
   `numeric_ambiguous` (fail-closed). Reste **inatteignable en prod** tant que ces 3 conditions ne sont pas réunies.
...
sont fermés, contrôle « une valeur propre se vérifie » intact. **Comptage corroboration réel désormais câblé**
(sources captées distinctes concordantes ; le résidu H3 « stub à 1 » est levé). Résidu documenté §8 = seulement
la contradiction inter-sources EXPLICITE (`no_disputed_claims` = V2 ; la divergence est déjà bloquée
implicitement par les buckets disjoints). SUGGESTION restantes non bloquantes : plages-mots « X à Y » entre deux
